...
export default function BookingPage({
  ...
  const [step, setStep] = useState<"details" | "date" | "confirmed" | "has-booking">("details");
  ...
  // Separate function to check for existing booking
  const checkForExistingBookingOnLoad = async () => {
    ...
    try {
      // First check localStorage for stored booking email
      const storedBookingEmail = localStorage.getItem(`booking_email_${interviewerId}`);
      console.log("Checking for existing booking - localStorage email:", storedBookingEmail);
      
      // Also check URL params
      const urlParams = new URLSearchParams(window.location.search);
      const emailParam = urlParams.get('email');
      console.log("Checking for existing booking - URL email:", emailParam);
      
      // Use email from localStorage or URL params
      const emailToCheck = storedBookingEmail || emailParam;
      console.log("Email to check:", emailToCheck, "Interviewer ID:", interviewerId);
      
      if (emailToCheck) {
        const bookingsRef = collection(db, "bookings");
        const emailQuery = query(
          bookingsRef,
          where("applicantEmail", "==", emailToCheck),
          where("interviewerId", "==", interviewerId),
          where("status", "in", ["CONFIRMED", "PENDING"])
        );
        const emailBookingsSnapshot = await getDocs(emailQuery);
        console.log("Found bookings:", emailBookingsSnapshot.docs.length);
        
        const existingFutureBooking = emailBookingsSnapshot.docs.find((doc) => {
          const data = doc.data();
          const bookingStartTime = data.startTime?.toDate();
          const isFutureBooking = bookingStartTime && isFuture(bookingStartTime);
          console.log("Booking check:", {
            id: doc.id,
            startTime: bookingStartTime,
            isFuture: isFutureBooking
          });
          return isFutureBooking;
        });

        if (existingFutureBooking) {
          console.log("Found existing future booking!");
        ...
        console.log("No email found in localStorage or URL");
      }
    } catch (error) {
      console.error("Error checking existing booking on load:", error);
    }
    
    // If no existing booking found, proceed to details form
    console.log("Setting step to details");
    setCheckingExistingBooking(false);
    setStep("details");
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await checkForExistingBookingOnLoad();
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store email in localStorage for future booking detection (persists across tabs)
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(`booking_email_${interviewerId}`, formData.email);
          console.log("✅ Stored email in localStorage:", formData.email, "for interviewer:", interviewerId);
          // Also update URL with email parameter (this ensures the link works when shared)
          const newUrl = `${window.location.pathname}?email=${encodeURIComponent(formData.email)}`;
          window.history.replaceState({}, '', newUrl);
          console.log("✅ Updated URL with email parameter:", newUrl);
        } catch (error) {
          console.error("❌ Error storing booking email:", error);
        }
      }
      setStep("confirmed");
    } catch (error) {
  ...
  const cancelExistingBooking = async () => {
    ...
    try {
      ...
      // Clear localStorage
      if (typeof window !== 'undefined') {
        localStorage.removeItem(`booking_email_${interviewerId}`);
      }
  ...
  const checkExistingBooking = async (email: string) => {
    ...
    try {
      ...
      if (existingFutureBooking) {
        ...
        // No existing booking found, proceed to date selection
        setStep("date");
      }
    } catch (error) {
      console.error("Error checking booking:", error);
      // On error, proceed to date selection
      setStep("date");
    } finally {
  ...
  if (loading || checkingExistingBooking) {
    return (
      ...
      <div className="min-h-screen bg-[#050507] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="relative rounded-2xl sm:rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl p-6 sm:p-12 text-center max-w-lg w-full">
          ...
          <div className="space-y-3">
            <button
              onClick={() => {
                // Cancel existing booking and start new booking flow
                cancelExistingBooking();
                setStep("details");
              }}
              disabled={cancelling}
              className="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-lg sm:rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white text-sm sm:text-base font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-violet-500/30 transition-all disabled:opacity-50"
            >
              {cancelling ? (
                ...
                <>
                  ...
                  <span className="text-sm sm:text-base">Reschedule Interview</span>
                </>
              )}
            </button>
            ...
            >
              Can&apos;t see meeting link? Check with your email
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#050507] relative overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-6 sm:py-12">
        ...
        {/* Check Booking Banner - only show on date selection step */}
        {step === "date" && (
            ...
            {step === "details" ? (
              <div className="rounded-2xl sm:rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl overflow-hidden">
                <div className="p-4 sm:p-6 border-b border-white/10">
                  ...
                  <p className="text-white/40 text-xs sm:text-sm mt-1">
                    Provide your information to start booking
                  </p>
                </div>
                <div className="p-4 sm:p-6">
                  <form onSubmit={handleDetailsSubmit} className="space-y-4 sm:space-y-5">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-4">
                      <button
                        type="submit"
                        className="flex-1 px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white text-sm sm:text-base font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-violet-500/30 transition-all"
                      >
                  ...
                  {selectedTime && (
                    <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/10 flex justify-end">
                      <button
                        ...
                        className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white text-sm sm:text-base font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-violet-500/30 transition-all disabled:opacity-50"
                      >
                        {submitting ? (
                          <>
                            <Loader2 className="w-4 h-4 animate-spin" />
